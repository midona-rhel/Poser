...
public static class BoneMatrixBuilder
{
    ...
    private static bool IsSuppressed(BoneDescriptor bone, bool showNsfw)
        => !showNsfw
        ...
        string? filter = null)
    {
        ...
        foreach (var row in AnamnesisMatrixTable.Rows)
        {
            ...
            foreach (var (name, pillLabel) in row.Pills)
            {
                ...
                var matches = skeleton.Bones.Where(bone =>
                    !bone.IsHidden && !IsSuppressed(bone, showNsfw) &&
        ...
        AppendGenerated(vm, skeleton, selection, covered, showNsfw);
        ...
        HashSet<(string BoneName, int PartialId)> covered, bool showNsfw)
    {
        ...
        {
            if (bone.IsHidden || IsSuppressed(bone, showNsfw) ||
