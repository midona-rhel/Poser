...
public static class AppShellView
{
    ...
    private static void DrawRow(AppShellViewModel vm, ShellSidebarRow row, Vector2 cursor, float innerW, float s, ImDrawListPtr dl, string id)
    {
        ...
        ViewText.Label(new Vector2(x, cursor.Y + 5f * s), row.Label, 13f, FontWeight.Regular, TextPrimary);
        ImGui.PopClipRect();

        if (row.Count.Length > 0)
            ViewText.Label(new Vector2(cursor.X + innerW - 8f * s - ViewText.Measure(row.Count, 11f, mono: true), cursor.Y + 7f * s),
