...
public record struct ElementStyle
{
    ...
    // Per-side border colors (border-top-color etc.); null sides fall back to BorderColor.
    ...
    public ElementStyle MergedWith(in ElementStyle overlay)
    {
        ...
        if (overlay.BorderTopColor.HasValue)    r.BorderTopColor = overlay.BorderTopColor;
        if (overlay.BorderRightColor.HasValue)  r.BorderRightColor = overlay.BorderRightColor;
        if (overlay.BorderBottomColor.HasValue) r.BorderBottomColor = overlay.BorderBottomColor;
        if (overlay.BorderLeftColor.HasValue)   r.BorderLeftColor = overlay.BorderLeftColor;
        ...
        if (overlay.BackgroundImage != null)   r.BackgroundImage = overlay.BackgroundImage;
        if (overlay.BackgroundImageFit.HasValue) r.BackgroundImageFit = overlay.BackgroundImageFit;
        if (overlay.BackgroundSvg != null)     r.BackgroundSvg = overlay.BackgroundSvg;
        ...
        if (overlay.Transform.HasValue)        r.Transform = overlay.Transform;
        if (overlay.ZIndex.HasValue)           r.ZIndex = overlay.ZIndex;
        if (overlay.DragHoverColor.HasValue)   r.DragHoverColor = overlay.DragHoverColor;
        ...
        if (overlay.FontWeight.HasValue)       r.FontWeight = overlay.FontWeight;
