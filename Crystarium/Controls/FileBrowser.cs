...
public class FileBrowser
{
    ...
    private void DrawContent(ImDrawListPtr drawList)
    {
        ...
        float bottomBarHeight = _isSaveMode
            ? (Flex.RowHeight * scale * 2 + Flex.RowSpacing * scale * 2)
            : (Flex.RowHeight * scale + Flex.RowSpacing * scale);
    ...
    {
        ImGui.TextColored(UIColors.Gray, "Quick Access");
        ImGui.Spacing();
        ...
        foreach (var fav in _favorites)
        {
            using (ImRaii.PushFont(UiBuilder.IconFont))
            {
                ImGui.TextColored(UIColors.Orange, fav.Icon.ToIconString());
            }
            ...
            if (isCurrentPath)
            {
                ImGui.TextColored(UIColors.SelectionActive, fav.Name);
            }
            else
            {
    ...
    private void DrawFileEntries()
    {
        for (int i = 0; i < _entries.Count; i++)
        {
            ...
            using (ImRaii.PushFont(UiBuilder.IconFont))
            {
                ...
                var iconColor = entry.IsDirectory ? UIColors.Orange : UIColors.Text;
    ...
    private void DrawBottomBar()
    {
        ...
        {
            ImGui.TextColored(UIColors.Red, _lastError);
        }
