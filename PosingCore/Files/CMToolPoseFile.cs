...
/// transcribed from Brio's <c>Files/CMToolPoseFile.cs</c>). Bones are hex-string
/// encoded rotation quaternions and scales — .cmp carries NO positions, so the
/// import path forces <c>ApplyPosition = false</c> (Brio leaves this to a popup
/// choice; forcing it can never zero a pose).
/// </summary>
[Serializable]
public class CMToolPoseFile
{
    private enum CmpRace : byte
    {
        Hyur = 1, Elezen = 2, Lalafel = 3, Miqote = 4,
        Roegadyn = 5, AuRa = 6, Hrothgar = 7, Viera = 8,
    }
    ...
    public static CMToolPoseFile? Load(string path)
    {
        try
        {
            return JsonSerializer.Deserialize<CMToolPoseFile>(File.ReadAllText(path), SerializerOptions);
        }
        catch
        {
            return null;
    ...
    /// <summary>
    /// Converts to a Brio-shaped <see cref="PoseFile"/>: reflect over the bone
    /// properties, decode hex strings, translate Anamnesis names to game names.
    /// Race-gated bones (Hroth*/Viera*) are skipped for other races.
    /// </summary>
    public PoseFile Upgrade()
    {
        ...
        {
            var boneName = propertyInfo.Name;
            if (boneName is "Race" or "Type" || boneName.EndsWith("Size"))
                continue;
            if (boneName.StartsWith("Hroth") && fileRace != CmpRace.Hrothgar)
                continue;
            if (boneName.StartsWith("Viera") && fileRace != CmpRace.Viera)
                continue;

            var rotString = (string?)propertyInfo.GetValue(this);
            var scaleString = (string?)GetType().GetProperty(boneName + "Size")?.GetValue(this);
            if (rotString == null && scaleString == null)
                continue;

            poseFile.Bones[AnamnesisBoneNameConverter.ToGame(boneName)] = StringToBone(rotString, scaleString);
        }

        return poseFile;
    }

    private static PoseFile.BoneData StringToBone(string? rot, string? scale)
    {
        var bone = new PoseFile.BoneData { Rotation = Quaternion.Identity };
