...
| Target naming and interface policy | Current docs still say `Poser.Core` and `Poser.Runtime`, while the project is `Poser.Core` and the native project is `Poser.Game`. | `product-and-boundaries.md` names Domain, Application, current `Poser.Game`, conditional host-free Persistence, current Poser.UI, and Host; `posing-runtime.md` names the native boundary. Keep `Poser.Game`, do not add `Poser.Runtime` or `Poser.UI.Kernel`, and retain concrete owners with interfaces only at real runtime/storage seams. |
