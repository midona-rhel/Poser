...
| Reference images overlay | Ktisis `ReferenceImage` entity + `Editor.ReferenceImages` | **fixed** — reference pictures are floating, aspect-locked, opacity-dimmed windows that join the Overlays group and persist across leaving GPose (`Poser/UI/Windows/ReferenceImageWindow.cs`, `ReferenceImageSession.cs`, `ReferenceImageGeometry.cs`, `Poser.Core/Config/ReferenceImageConfiguration.cs`) |
