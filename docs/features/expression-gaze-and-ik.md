...
targets another scene actor by stable id and requires an explicit choice.
Disabling a part immediately restores its pre-Poser native target (captured
once, never re-seeded from Poser output); Off writes nothing. State keys by
`GameObjectId`, so redraws cannot orphan it; a vanished target transitions to
Off. Reset restores every part and clears gaze state.
...
solver target; rotation/scale never solve; nothing persists — undo and export
stay pure deltas. Arming is per selected bone via `ConfigureIk`, gated to the
four chain ends (`j_te_l/r`, `j_asi_d_l/r`); no actor-wide arming exists.
