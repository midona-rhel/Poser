...
# Demonstrates that one cross-component batch is pixel-identical to fully
# isolated one-process-per-capture runs. Every entry still owns a fresh
# ImGui context over the shared atlas; PNG hashes must match pairwise.
...
$entries = @(foreach ($name in $Components) {
...
foreach ($entry in $entries) {
    & $exe $entry.Name $entry.Isolated `
        $entry.Scale.ToString($invariant) $entry.Theme | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw "Isolated capture failed for $($entry.Name), $($entry.Theme) at $($entry.Scale)."
    }
}
$batchFile = Join-Path $work "cross-component-batch.txt"
$entries | ForEach-Object {
    "$($_.Name)`t$($_.Batched)`t$($_.Scale.ToString($invariant))`t$($_.Theme)"
} | Set-Content -Encoding utf8 -LiteralPath $batchFile
& $exe --batch $batchFile | Out-Null
if ($LASTEXITCODE -ne 0) {
    throw "Cross-component batch capture failed."
}
...
foreach ($entry in $entries) {
    ...
    if ($isolated -ne $batched) {
        $mismatches += "$($entry.Name) @$($entry.Theme)@$($entry.Scale)"
    }
}

if ($mismatches.Count -gt 0) {
    Write-Host "BATCH ISOLATION FAILED for $($mismatches.Count)/$($entries.Count) pairs:"
    $mismatches | ForEach-Object { Write-Host "  $_" }
    exit 1
}
Write-Host "Batch isolation verified: $($entries.Count)/$($entries.Count) capture pairs hash-identical."
exit 0
